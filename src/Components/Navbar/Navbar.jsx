import { FaArrowDown } from "react-icons/fa";
import { NavLink } from "react-router-dom";
import cv from '../../../public/images/cv.pdf';


const Navbar = () => {
    const navLink = <>
    <NavLink to='/'><li><a>Home</a></li></NavLink>
    <NavLink to='/projects'><li><a>Projects</a></li></NavLink>
    <NavLink to='/contact'><li><a>Contact</a></li></NavLink>
    <NavLink to='/about'><li><a>About</a></li></NavLink>
    </>
    return (
        <div className="navbar bg-fuchsia-100">
            <div className="navbar-start">
                <div className="dropdown">
                    <div tabIndex={0} role="button" className="btn btn-ghost lg:hidden">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /></svg>
                    </div>
                    <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52">
                        
                        {navLink}
                    </ul>
                </div>
                <a className="btn btn-ghost text-xl">Portfolio</a>
            </div>
            <div className="navbar-center hidden lg:flex">
                <ul className="menu menu-horizontal px-1">
                    {navLink}
                </ul>
            </div>
            <div className="navbar-end">
            <a href={cv}><button className='px-5 py-3 bg-fuchsia-500 text-white rounded-xl font-bold'>Download CV <FaArrowDown className="inline"></FaArrowDown></button></a>
            </div>
        </div>
    );
};

export default Navbar;